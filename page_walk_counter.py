...
Deletions, separated by comma, unit: run# (starting from #1, this affacts partitions)
Clear flag
...
def print_usage_and_exit(args):
    print("Usage:", args[0], "perf.log", "[-p partitions]", "[-e exclusions]", "[-d deletions]", "[-c]", "[-o output.log]", "[-v]")
    sys.exit(-1)
...
while True:
    ...
    if eval_count in deletion:
        deleted += [eval_count]
        del_lines += list(range(this_stats[5], this_stats[6] + 1))
    if eval_count not in exclusion and eval_count not in deletion and this_runtime:
